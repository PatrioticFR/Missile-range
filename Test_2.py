...
class Missile:
    ...
    def trouver_alpha_optimal(self):
        ...
        print("Première passe d'optimisation...")
        for alpha_deg in np.arange(alpha_min, alpha_max, 2):  # Pas de 2° pour accélérer
            ...
            if self.altitudes[-1] <= self.h_cible:
                ...
                score = portee - marge_batterie * 100  # Pénaliser les trajectoires qui n'utilisent pas toute la batterie

                if cible_atteinte and score > portee_max:
                    portee_max = score
                    alpha_optimal = alpha_deg
                    print(
                        f"Trouvé: alpha={alpha_deg}°, portée={portee:.2f}m, durée={duree_vol:.2f}s (batterie={self.batterie}s)")
        ...
        for alpha_deg in np.arange(max(alpha_min, alpha_optimal - 2), min(alpha_max, alpha_optimal + 2),
            ...
            # Vérifier si la cible a été atteinte
            cible_atteinte = False
            duree_vol = len(self.altitudes) * 0.1  # Durée en secondes
            ...
            if self.altitudes[-1] <= self.h_cible:
                ...
                # On veut minimiser cette marge pour atteindre la cible au moment où la batterie s'épuise
                score = portee - marge_batterie * 100

                if cible_atteinte and score > portee_max:
                    portee_max = score
                    alpha_optimal = alpha_deg
                    print(
                        f"Affiné: alpha={alpha_deg}°, portée={portee:.2f}m, durée={duree_vol:.2f}s (batterie={self.batterie}s)")
        ...
        portee_reelle = self.calculer_portee(generer_graphiques=False)
...
print(f"L'angle d'attaque optimal est de {alpha_optimal} degrés, ce qui donne une portée de {portee_max:.2f} mètres.")
