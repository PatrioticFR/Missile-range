...
def main_advanced_example():
    ...
    # CHANGEZ ICI LE MISSILE SOUHAITÉ
    SELECTED_MISSILE = "MICA"  # Options: "MICA", "AIM-120B", "R-77"

    # Récupération de la configuration
    missile_config = MissileDatabase.get_missile_config(SELECTED_MISSILE)

    print("🎯 SIMULATEUR DE MISSILE OPTIMISÉ")
    print("=" * 50)
    print(f"Configuration: {SELECTED_MISSILE}")
    print(f"Masse initiale: {missile_config.initial_mass} kg")
    print(f"Altitude de lancement: {missile_config.initial_altitude} m")
    print(f"Altitude cible: {missile_config.target_altitude} m")
    print(f"Longueur totale: {missile_config.total_length} m")
    print(f"Autonomie batterie: {missile_config.battery_life} s")

    # 1. Optimisation standard
    print("\n🚀 1. OPTIMISATION STANDARD")
    simulator = MissileSimulator(missile_config)
    optimizer = TrajectoryOptimizer(simulator)
    ...
    print("\n🔬 2. ÉTUDE PARAMÉTRIQUE")
    campaign = OptimizationCampaign(missile_config)
    ...
    print("\n📈 3. ANALYSE DE SENSIBILITÉ")
    sensitivity_analysis(missile_config, best_params)

    print("\n🔍 4. COMPARAISON DE TOUS LES MISSILES")
    compare_missiles()
