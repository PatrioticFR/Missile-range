...
Atention etude paramétrique nécessite 100go+ de RAM. Ne pas faire sinon (crash)
"""
...
class OptimizationCampaign:
    """
    Amélioration: Gestionnaire de campagnes pour analyses paramétriques
    """
    ...
    def run_parametric_study(self, parameter_variations: dict,
        ...
        """
        Amélioration: Étude paramétrique automatisée
        """
        results = {}
        ...
        for param_name, values in parameter_variations.items():
            ...
            for value in values:
                ...
                try:
                    ...
                    if verbose:
                        print(f"   {param_name}={value}: {best_result.range_m:.0f}m "
                              f"({'✅' if best_result.target_reached else '❌'})")
    ...
    def _modify_config(self, param_name: str, value: float) -> MissileConfiguration:
        """
        Amélioration: Modification dynamique de configuration
        """
    ...
    def generate_parametric_plots(self, results: dict):
        """
        Amélioration: Visualisation des études paramétriques
        """
        ...
        for i, (param_name, param_results) in enumerate(results.items()):
            ...
            # Graphique de portée
            axes[0, i].plot(values, ranges, 'bo-', linewidth=2, markersize=6)
            ...
            axes[1, i].bar(values, success_rate, alpha=0.7, color='green')
            axes[1, i].set_xlabel(param_name)
            axes[1, i].set_ylabel('Succès (cible atteinte)')
            axes[1, i].set_title(f'Taux de succès vs {param_name}')
            axes[1, i].set_ylim(0, 1.1)
            axes[1, i].grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig(f'parametric_study_{int(time.time())}.png', dpi=300, bbox_inches='tight')
        plt.close('all')
        print(f"📊 Étude paramétrique sauvegardée: parametric_study_{int(time.time())}.png")
...
def main_advanced_example():
    ...
    variations = {
        'initial_velocity': [250, 275, 300, 325, 350],
        'battery_life': [60, 65, 70, 75, 80],
        'wing_surface': [0.015, 0.020, 0.025, 0.030],
    }
