...
class Missile:
    ...
    def trouver_meilleure_combinaison(self,
        ...
        def executer_evaluation_parallel(combinaisons) -> List[Tuple[float, float, float, float, float]]:
            with Pool(processes=cpu_count()) as pool:
                resultats = pool.map(_eval_comb_wrapper, [(self, *comb) for comb in combinaisons])
        ...
        valid_1 = executer_evaluation_parallel(combinaisons)
        ...
        valid_2 = executer_evaluation_parallel(toutes_combinaisons_fines)
        ...
        end_time = time.time()
        print(f"\nDurée de la simulation : {end_time - start_time:.2f} secondes")
...
if __name__ == "__main__":
    ...
    freeze_support()  # facultatif mais recommandé sur Windows
