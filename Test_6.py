...
class Missile:
    ...
    def calculate_drag_coefficients(self, M, l_n, d, l, A_e, S_Ref, q, alpha):
        ...
        CD_Wing = 0.1 * (self.S_wing / S_Ref) * (abs(math.sin(math.radians(alpha))) ** 2)
        if M > 1:
            CD_Wing += 0.05 * (self.S_wing / S_Ref)
    ...
    def calculate_normal_force_coefficient(self, a, b, phi, alpha, l, d):
        ...
        CN_wing = 2 * math.pi * math.radians(alpha) * (self.S_wing / (math.pi * (d / 2) ** 2))
    ...
    def trouver_meilleure_combinaison(self, angle_tir_min=-45, angle_tir_max=45, alpha_asc_min=-45, alpha_asc_max=45,
                                     alpha_desc_min=-45, alpha_desc_max=45, pas_tir_grossier=1, pas_alpha_grossier=1,
